...
with st.sidebar:
    ...
    # Prefer our multi-user auth email (session_state["user_email"]); fall back
    # to the legacy bitefit_user dict and Streamlit's native experimental_user.
    ...
    _display_email = _user_email_display or _streamlit_email
    if _display_email:
...
if not run_btn and "last_plan" not in st.session_state:
    ...
    # Behind a feature flag (default OFF) so the existing calorie dashboard
    # remains the unchanged production view for everyone else. When ON, a
    # small Streamlit toggle (חלבון | קלוריות) lets the user switch which
    # ring leads the home tab; the calorie dashboard remains accessible.
    try:
        from nutrition_app.feature_flags import FF_PROTEIN_FIRST_WIDGET as _FF_PROTEIN
    except Exception:
        _FF_PROTEIN = False
    if _FF_PROTEIN:
        try:
            from nutrition_app.macro_calculator import (
                protein_floor_g as _protein_floor_g,
                protein_ring_color as _protein_ring_color,
            )
            from nutrition_app.repositories.user_meal_preferences_repository import (
                UserMealPreferencesRepository as _UMPRepo,
            )
            _prefs_for_widget = _UMPRepo().load(_DASH_USER)
            _prim = (_prefs_for_widget.primary_metric
                     if _prefs_for_widget is not None else "calories")
            _tab_label_protein = "חלבון"
            _tab_label_calories = "קלוריות"
            _view_choice = st.radio(
                "תצוגה ראשית",
                options=[_tab_label_protein, _tab_label_calories],
                index=0 if _prim == "protein" else 1,
                horizontal=True,
                key="ff_protein_primary_toggle",
                label_visibility="collapsed",
            )
            if _view_choice == _tab_label_protein:
                _p_floor = _protein_floor_g(_user_obj)
                _p_ratio = (float(prot_eaten) / _p_floor) if _p_floor > 0 else 0.0
                _p_color = _protein_ring_color(_p_ratio)
                _pr2, _pcx2, _pcy2, _psz2, _psw2 = 68, 80, 80, 160, 12
                _pc2 = 2 * _math.pi * _pr2
                ...
                _protein_widget_html = (
                    ...
                    f'</div>'
                    # right-side ring
                    f'<div dir="rtl" style="position:relative;width:{_psz2}px;height:{_psz2}px;'
                    f'flex-shrink:0">'
                    ...
                    f'stroke-dashoffset="{_pc2*0.25:.1f}" stroke-linecap="round"/>'
                    f'</svg>'
                    f'<div dir="rtl" style="position:absolute;inset:0;display:flex;'
                ...
                # Render ABOVE the calorie ring (next st.markdown call).
                st.markdown(_protein_widget_html, unsafe_allow_html=True)
        except Exception:
...
# ── Calm Mode banner helper (additive) ──────────────────────────────
def _calm_mode_banner_text() -> str:
    """Read the calm-mode banner string from config/strings_he.json."""
    _fallback = "אצלנו אין סטריקים. אין פרסומות. אין עונשים. אתה בשליטה."
    try:
        import json as _json_cm
        _strs_path = os.path.join(
            os.path.dirname(os.path.abspath(__file__)), "config", "strings_he.json"
        )
        with open(_strs_path, "r", encoding="utf-8") as _f_cm:
            return _json_cm.load(_f_cm).get("home.calm_mode_banner", _fallback)
    except Exception:
        return _fallback

try:
    from nutrition_app.feature_flags import FF_CALM_MODE_DEFAULT as _FF_CALM_TARGETS
except Exception:
    _FF_CALM_TARGETS = False
...
with tab_targets:
    # Calm Mode banner — additive, gated by FF_CALM_MODE_DEFAULT, once per session
    if _FF_CALM_TARGETS and not st.session_state.get("_calm_banner_targets_shown"):
        st.info(_calm_mode_banner_text())
        st.session_state["_calm_banner_targets_shown"] = True
